            ...
            if (item.STAT_Name === CONDITION || CONDITION === "ALL") {
                display += `
                    <tr id="data">
                        ...
                        <td id="status" class="${item.STAT_Name}-status">
                            <p>${item.STAT_Name}</p>
                        </td>
                        <td class="action">`;
        
                if (item.STAT_Name !== "ACTIVE") {
                    display += `
...
function StatPatient(StatID, AccountID){
    ...
    .then(data => {
        if (data.success) {
            ...
            Swal.fire({
                title: 'Success!',
                text: StatID == 2 ? 'Patient Account has been Denied Access successfully.' : 'Patient Account has been Gained Access successfully.',
                icon: 'success',
